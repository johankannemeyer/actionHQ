...
function TeamPerformanceCharts({ teamName, matches, sortOrder }: { teamName: string; matches: MatchActivity[]; sortOrder: TeamStatsSort }) {
  ...
  const dotAvoiders = intelligenceRows.filter((player) => player.balls > 0 && player.matches.size / gamesPlayed >= FINALS_QUALIFY_THRESHOLD).sort((a, b) => a.dots / a.balls - b.dots / b.balls).slice(0, 5);
  const wicketResilience = intelligenceRows.filter((player) => player.balls > 0 && player.matches.size / gamesPlayed >= FINALS_QUALIFY_THRESHOLD).sort((a, b) => (b.dismissals ? b.balls / b.dismissals : b.balls + 1000) - (a.dismissals ? a.balls / a.dismissals : a.balls + 1000)).slice(0, 5);
  const runRotators = intelligenceRows.filter((player) => player.balls > 0 && player.matches.size / gamesPlayed >= FINALS_QUALIFY_THRESHOLD).sort((a, b) => b.rotations / b.balls - a.rotations / a.balls).slice(0, 5);
  const wicketTakers = intelligenceRows.filter((player) => player.bowlingBalls > 0 && player.matches.size / gamesPlayed >= FINALS_QUALIFY_THRESHOLD).sort((a, b) => b.wickets - a.wickets).slice(0, 5);
  const topWicketTaker = Math.max(...wicketTakers.map((player) => player.wickets), 1);
  const dotBallCreators = intelligenceRows.filter((player) => player.bowlingBalls > 0 && player.matches.size / gamesPlayed >= FINALS_QUALIFY_THRESHOLD).sort((a, b) => b.bowlingDots / b.bowlingBalls - a.bowlingDots / a.bowlingBalls).slice(0, 5);
  const bowlingDiscipline = intelligenceRows.filter((player) => player.bowlingBalls > 0 && player.matches.size / gamesPlayed >= FINALS_QUALIFY_THRESHOLD).sort((a, b) => a.extras / a.bowlingBalls - b.extras / b.bowlingBalls).slice(0, 5);
  ...
  const runRateSeries = analysed.map((match) => {
    const ourInnings = (match.innings ?? []).find((inn) => inn.battingTeam.toLowerCase() === teamName.toLowerCase());
    const theirInnings = (match.innings ?? []).find((inn) => inn.battingTeam.toLowerCase() !== teamName.toLowerCase());
    const ourOvers = ourInnings ? (ourInnings.pairs ?? []).reduce((sum, pair) => sum + (pair.overs?.length ?? 0), 0) : 0;
    const theirOvers = theirInnings ? (theirInnings.pairs ?? []).reduce((sum, pair) => sum + (pair.overs?.length ?? 0), 0) : 0;
    return { id: match.id, opponent: match.opponent, ourRate: ourOvers ? Math.round(match.scored / ourOvers * 100) / 100 : 0, theirRate: theirOvers ? Math.round(match.conceded / theirOvers * 100) / 100 : 0 };
  });
  const maxRunRate = Math.max(...runRateSeries.flatMap((match) => [match.ourRate, match.theirRate]), 1);
  const runRatePxPerPoint = 78;
  const runRateChartWidth = Math.max(560, (runRateSeries.length - 1) * runRatePxPerPoint + 40);
  const runRateCoord = (rate: number, index: number) => [runRateSeries.length > 1 ? 20 + index / (runRateSeries.length - 1) * (runRateChartWidth - 40) : runRateChartWidth / 2, 100 - Math.min(96, rate / maxRunRate * 90)];
  const runRatePoint = (rate: number, index: number) => runRateCoord(rate, index).join(",");
  ...
  const runRatePathFor = smoothPath(runRateSeries.map((match, index) => runRateCoord(match.ourRate, index)));
  const runRatePathAgainst = smoothPath(runRateSeries.map((match, index) => runRateCoord(match.theirRate, index)));
  const runRateAreaFor = runRateSeries.length ? `${runRatePathFor} L${runRateCoord(0, runRateSeries.length - 1)[0]},100 L20,100 Z` : "";
  const runRateAreaAgainst = runRateSeries.length ? `${runRatePathAgainst} L${runRateCoord(0, runRateSeries.length - 1)[0]},100 L20,100 Z` : "";
  const runRateYTicks = [100, 75, 50, 25, 0].map((y) => ({ y, value: Math.round(maxRunRate * (100 - y) / 90 * 10) / 10 }));
  ...
  const rankRows = (rows: TeamPlayerIntelligence[], value: (player: TeamPlayerIntelligence) => string, width: (player: TeamPlayerIntelligence) => number) => rows.length ? rows.map((player, index) => <div className="intelligence-rank-row" key={`${playerKey(player.name)}-${index}`}><b>{index + 1}</b><Initials name={player.name} /><span><strong>{player.name}</strong><i><em style={{ width: `${Math.max(4, Math.min(100, width(player)))}%` }} /></i></span><em>{value(player)}</em></div>) : <div className="panel-empty">No players qualify yet — this ranking needs {Math.round(FINALS_QUALIFY_THRESHOLD * 100)}% of games played, same as the season finals qualification.</div>;
  ...
  return <div className="team-intelligence-suite">
    ...
    <section className="team-ranking-section"><div className="section-title"><div><p className="overline">BOWLING INTELLIGENCE</p><h2>Who creates the pressure?</h2></div><span>Calculated from every recorded delivery</span></div><div className="team-ranking-grid">
      <article><header><span>01</span><div><strong>Wicket Takers</strong><small>Most wickets claimed</small></div></header>{rankRows(wicketTakers, (player) => `${player.wickets} wkts`, (player) => player.wickets / topWicketTaker * 100)}</article>
      <article><header><span>02</span><div><strong>Dot Ball Creators</strong><small>Highest percentage of dot balls bowled</small></div></header>{rankRows(dotBallCreators, (player) => `${Math.round(player.bowlingDots / player.bowlingBalls * 100)}% dots`, (player) => player.bowlingDots / player.bowlingBalls * 100)}</article>
      <article><header><span>03</span><div><strong>Bowling Discipline</strong><small>Fewest extras per delivery bowled</small></div></header>{rankRows(bowlingDiscipline, (player) => `${Math.round(player.extras / player.bowlingBalls * 100)}% extras`, (player) => 100 - player.extras / player.bowlingBalls * 100)}</article>
    </div>
    </section>

    <section className="team-pattern-grid">
      <article className="consistency-card run-rate-card"><header><div><p>RUN RATE TREND</p><h2>Our run rate vs opponent</h2></div><div className="chart-legend"><i className="legend-for" />Our RR<i className="legend-against" />Opponent RR</div></header><div className="run-rate-chart"><div className="run-rate-axis">{runRateYTicks.map((tick) => <span key={tick.y}>{tick.value}</span>)}</div><div className="run-rate-scroll"><div className="run-rate-plot" style={{ width: `${runRateChartWidth}px` }}><svg viewBox={`0 0 ${runRateChartWidth} 100`} preserveAspectRatio="none" role="img" aria-label={`${teamName} run rate versus opponent run rate across ${runRateSeries.length} matches`}>
        <defs>
          <linearGradient id="rrGradFor" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--green)" stopOpacity="0.16" /><stop offset="100%" stopColor="var(--green)" stopOpacity="0" /></linearGradient>
          <linearGradient id="rrGradAgainst" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--orange)" stopOpacity="0.14" /><stop offset="100%" stopColor="var(--orange)" stopOpacity="0" /></linearGradient>
        </defs>
        {[0, 25, 50, 75, 100].map((y) => <line key={y} x1="0" x2={runRateChartWidth} y1={y} y2={y} className="run-rate-grid" />)}
        <path className="run-rate-area" d={runRateAreaAgainst} fill="url(#rrGradAgainst)" />
        <path className="run-rate-area" d={runRateAreaFor} fill="url(#rrGradFor)" />
        <path className="run-rate-line run-rate-line-against" d={runRatePathAgainst} />
        <path className="run-rate-line run-rate-line-for" d={runRatePathFor} />
        {runRateSeries.map((match, index) => <circle key={`${match.id}-against`} className="run-rate-dot run-rate-dot-against" r="5" cx={runRatePoint(match.theirRate, index).split(",")[0]} cy={runRatePoint(match.theirRate, index).split(",")[1]} />)}
        {runRateSeries.map((match, index) => <circle key={`${match.id}-for`} className="run-rate-dot run-rate-dot-for" r="5" cx={runRatePoint(match.ourRate, index).split(",")[0]} cy={runRatePoint(match.ourRate, index).split(",")[1]} />)}
      </svg>{runRateSeries.map((match, index) => <div className="run-rate-hover" key={match.id} style={{ left: `${runRateCoord(0, index)[0] - runRatePxPerPoint / 2}px`, width: `${runRatePxPerPoint}px` }}><span className="run-rate-guide" style={{ left: `${runRatePxPerPoint / 2}px` }} /><div className="run-rate-tooltip" style={{ left: `${runRatePxPerPoint / 2}px` }}><strong>{`vs ${match.opponent}`}</strong><span className="rr-tt-for"><i />{teamName}<b>{match.ourRate} rpo</b></span><span className="rr-tt-against"><i />{match.opponent}<b>{match.theirRate} rpo</b></span></div></div>)}</div><div className="run-rate-labels" style={{ width: `${runRateChartWidth}px` }}>{runRateSeries.map((match, index) => <span key={match.id} title={match.opponent} style={{ left: `${runRateCoord(0, index)[0]}px` }}>{index + 1}</span>)}</div></div></div></article>
      <article className="wickets-trend-card"><header><div><p>WICKETS &amp; EXTRAS</p><h2>Per match breakdown</h2></div><div className="chart-legend combo-legend"><i className="legend-taken" />Wkts taken<i className="legend-lost" />Wkts lost<i className="legend-extra-for" />Extras for<i className="legend-extra-against" />Extras against</div></header><div className="score-trend-chart" role="img" aria-label={`${teamName} wickets taken versus lost and extras for versus against per match`}>{matchWicketsExtras.map((match, index) => <div className="score-chart-column combo-column" key={match.id} style={{ "--stagger": `${index * 35}ms` } as CSSProperties}><div className="combo-tooltip"><strong>{teamName} vs {match.opponent}</strong><span><i className="legend-taken" />Wkts taken<b>{match.wicketsTaken}</b></span><span><i className="legend-lost" />Wkts lost<b>{match.wicketsLost}</b></span><span><i className="legend-extra-for" />Extras for<b>{match.extrasFor}</b></span><span><i className="legend-extra-against" />Extras against<b>{match.extrasAgainst}</b></span></div><div className="score-column-values combo-values"><span><b>{match.wicketsTaken}</b><small>{match.wicketsLost}</small></span><span><b>{match.extrasFor}</b><small>{match.extrasAgainst}</small></span></div><div className="score-column-bars combo-bars"><i className="bar-taken" style={{ height: `${Math.max(4, match.wicketsTaken / maxMatchWickets * 100)}%` }} /><i className="bar-lost" style={{ height: `${Math.max(4, match.wicketsLost / maxMatchWickets * 100)}%` }} /><i className="bar-extra-for" style={{ height: `${Math.max(4, match.extrasFor / maxMatchExtras * 100)}%` }} /><i className="bar-extra-against" style={{ height: `${Math.max(4, match.extrasAgainst / maxMatchExtras * 100)}%` }} /></div></div>)}</div></article>
...
export default function Home() {
  ...
  const directoryStatsFor = (item: PlayerProfile) => {
    if (playerDirectorySeasonFilter === "all") return { games: item.allTime.games, runs: item.allTime.runs, wickets: item.allTime.wickets, impact: item.allTime.contribution, strikeRate: item.allTime.strikeRate, scorecards: item.allTime.linkedMatches };
    const playerSeason = item.seasons.find((entry) => String(entry.teamSeasonId) === playerDirectorySeasonFilter);
    return { games: playerSeason?.games ?? 0, runs: playerSeason?.runs ?? 0, wickets: playerSeason?.wickets ?? 0, impact: playerSeason?.contribution ?? 0, strikeRate: playerSeason?.strikeRate ?? 0, scorecards: item.matches.filter((match) => String(match.teamSeasonId) === playerDirectorySeasonFilter).length };
  };
  const directoryFingerprintFor = (stats: { games: number; runs: number; wickets: number; impact: number }) => {
    const games = Math.max(1, stats.games);
    const averageRuns = Math.round(stats.runs / games * 10) / 10;
    const averageWickets = Math.round(stats.wickets / games * 100) / 100;
    const averageImpact = Math.round(stats.impact / games * 10) / 10;
    const battingScore = Math.min(100, Math.round(averageRuns / 17 * 100));
    const bowlingScore = Math.min(100, Math.round(averageWickets / 2 * 100));
    const impactScore = Math.min(100, Math.max(0, Math.round((averageImpact + 12) / 36 * 100)));
    return { battingScore, bowlingScore, impactScore, overall: Math.round((battingScore + bowlingScore + impactScore) / 3) };
  };
  ...
  const scorecardOurs = scorecardRows.filter((item) => !team || item.teamName.toLowerCase() === team.name.toLowerCase());
  const scorecardLeaderPool = scorecardOurs.length ? scorecardOurs : scorecardRows;
  const scorecardLeaders = {
    runs: [...scorecardLeaderPool].sort((a, b) => b.runs - a.runs)[0],
    wickets: [...scorecardLeaderPool].sort((a, b) => b.wickets - a.wickets)[0],
    contribution: [...scorecardLeaderPool].sort((a, b) => b.contribution - a.contribution)[0],
  };
  ...
  const scorecardOurBattingDeliveries = (scorecardMatch?.innings?.find((innings) => innings.battingTeam.toLowerCase() === (team?.name ?? "").toLowerCase())?.pairs ?? []).flatMap((pair) => pair.overs.flatMap((over) => over.deliveries ?? []));
  const scorecardRunTypes = { dots: 0, ones: 0, twos: 0, threes: 0, fours: 0, fives: 0, sixes: 0, sevensPlus: 0, dismissals: 0 };
  const scorecardDismissalCounts = new Map<string, number>();
  for (const delivery of scorecardOurBattingDeliveries) {
    const runs = deliveryRunValue(delivery.outcome);
    if (runs === 0) scorecardRunTypes.dots += 1;
    else if (runs === 1) scorecardRunTypes.ones += 1;
    else if (runs === 2) scorecardRunTypes.twos += 1;
    else if (runs === 3) scorecardRunTypes.threes += 1;
    else if (runs === 4) scorecardRunTypes.fours += 1;
    else if (runs === 5) scorecardRunTypes.fives += 1;
    else if (runs === 6) scorecardRunTypes.sixes += 1;
    else if ((runs ?? 0) >= 7) scorecardRunTypes.sevensPlus += 1;
    if (isDismissalOutcome(delivery.outcome)) {
      scorecardRunTypes.dismissals += 1;
      const label = dismissalLabel(delivery.outcome);
      scorecardDismissalCounts.set(label, (scorecardDismissalCounts.get(label) ?? 0) + 1);
    }
  }
  const scorecardRunTypeEntries = [
    { label: "Dots", value: scorecardRunTypes.dots, color: "#a8b3c4" }, { label: "1s", value: scorecardRunTypes.ones, color: "#63748a" }, { label: "2s", value: scorecardRunTypes.twos, color: "#347ff0" }, { label: "3s", value: scorecardRunTypes.threes, color: "#5f5bea" }, { label: "4s", value: scorecardRunTypes.fours, color: "#a248ed" }, { label: "5s", value: scorecardRunTypes.fives, color: "#e0357a" }, { label: "6s", value: scorecardRunTypes.sixes, color: "#00b3a4" }, { label: "7s+", value: scorecardRunTypes.sevensPlus, color: "#d9ef45" }, { label: "Outs", value: scorecardRunTypes.dismissals, color: "#ff4d0a" },
  ];
  const scorecardRunTypeTotal = Math.max(1, scorecardRunTypeEntries.reduce((sum, item) => sum + item.value, 0));
  let scorecardRunTypeStop = 0;
  const scorecardRunTypeGradient = scorecardRunTypeEntries.map((item) => { const start = scorecardRunTypeStop; scorecardRunTypeStop += item.value / scorecardRunTypeTotal * 100; return `${item.color} ${start}% ${scorecardRunTypeStop}%`; }).join(", ");
  const scorecardDismissalPalette = ["#ff4d0a", "#211515", "#8f6b5e", "#d38a6e", "#ffb69b", "#a99d96"];
  const scorecardDismissalEntries = [...scorecardDismissalCounts.entries()];
  const scorecardDismissalTotal = Math.max(1, scorecardDismissalEntries.reduce((sum, [, count]) => sum + count, 0));
  let scorecardDismissalStop = 0;
  const scorecardDismissalGradient = scorecardDismissalEntries.map(([, count], index) => { const start = scorecardDismissalStop; scorecardDismissalStop += count / scorecardDismissalTotal * 100; return `${scorecardDismissalPalette[index % scorecardDismissalPalette.length]} ${start}% ${scorecardDismissalStop}%`; }).join(", ");
      ...
      <section className="main-column">
        ...
        {view === "scorecard" && scorecardMatch && <>
          ...
          <section className="internal-scorecard">
            ...
            <div className="scorecard-result"><div><Initials name={scorecardMatch.homeTeam} large src={team && scorecardMatch.homeTeam.toLowerCase() === team.name.toLowerCase() ? team.imageUrl : null} /><span><b>{scorecardMatch.homeTeam}</b><small>{scorecardMatch.homeSkins} skins · {scorecardMatch.homePoints} league points</small></span><strong>{scorecardMatch.homeScore}</strong></div><i><span>{scorecardMatch.homeScore > scorecardMatch.awayScore ? `${scorecardMatch.homeTeam} won` : scorecardMatch.awayScore > scorecardMatch.homeScore ? `${scorecardMatch.awayTeam} won` : "Match tied"}</span><b>{Math.abs(scorecardMatch.homeScore - scorecardMatch.awayScore)} run margin</b></i><div><Initials name={scorecardMatch.awayTeam} large src={team && scorecardMatch.awayTeam.toLowerCase() === team.name.toLowerCase() ? team.imageUrl : null} /><span><b>{scorecardMatch.awayTeam}</b><small>{scorecardMatch.awaySkins} skins · {scorecardMatch.awayPoints} league points</small></span><strong>{scorecardMatch.awayScore}</strong></div></div>
          ...
          <div className="scorecard-highlights"><article><span>PLAYER OF THE MATCH</span><Initials name={(scorecardMatch.playerOfMatch || scorecardLeaders.contribution?.playerName || "Player").replace(/\s*\([^)]*\)\s*$/, "")} /><div><b>{scorecardMatch.playerOfMatch || scorecardLeaders.contribution?.playerName || "Not awarded"}</b><small>Official match award</small></div></article><article><span>TOP SCORE</span><div className="highlight-lead"><Initials name={scorecardLeaders.runs?.playerName ?? "Player"} /><strong>{scorecardLeaders.runs?.runs ?? 0}</strong></div><div><b>{scorecardLeaders.runs?.playerName ?? "—"}</b><small>{scorecardLeaders.runs?.teamName ?? ""}</small></div></article><article><span>MOST WICKETS</span><div className="highlight-lead"><Initials name={scorecardLeaders.wickets?.playerName ?? "Player"} /><strong>{scorecardLeaders.wickets?.wickets ?? 0}</strong></div><div><b>{scorecardLeaders.wickets?.playerName ?? "—"}</b><small>{scorecardLeaders.wickets?.teamName ?? ""}</small></div></article><article><span>TOP IMPACT</span><div className="highlight-lead"><Initials name={scorecardLeaders.contribution?.playerName ?? "Player"} /><strong>{scorecardLeaders.contribution?.contribution ?? 0}</strong></div><div><b>{scorecardLeaders.contribution?.playerName ?? "—"}</b><small>Contribution</small></div></article></div>

          <section className="match-intelligence-panel"><div className="section-title"><div><p className="overline">MATCH INTELLIGENCE</p><h2>How the game was won</h2></div><span>Runs, wickets, extras and pressure from every delivery</span></div><div className="match-verdict"><span>{scorecardWinnerAnalysis ? "DECIDING EDGE" : "MATCH BALANCE"}</span><strong>{scorecardWinnerAnalysis ? `${scorecardWinnerAnalysis.name} won by ${Math.abs(scorecardHomeAnalysis.runs - scorecardAwayAnalysis.runs)} runs` : "The scores finished level"}</strong><p>{scorecardWinnerAnalysis ? `${scorecardWinnerAnalysis.name} lost ${scorecardWinnerAnalysis.wicketsLost} wickets, gave away ${scorecardWinnerAnalysis.extrasGiven} extra deliveries and created ${scorecardWinnerAnalysis.dotPressure}% dot-ball pressure. ${scorecardWinnerAnalysis.extrasGiven < scorecardLoserAnalysis.extrasGiven ? `They conceded ${scorecardLoserAnalysis.extrasGiven - scorecardWinnerAnalysis.extrasGiven} fewer extras than ${scorecardLoserAnalysis.name}.` : `${scorecardLoserAnalysis.name} was more disciplined on extras, so the scoring and wicket margins made the difference.`}` : "Neither team created a final-score advantage; compare wickets, extras and skins below to see where momentum shifted."}</p></div><div className="match-intelligence-kpis"><article><span>RUN MARGIN</span><strong>{Math.abs(scorecardHomeAnalysis.runs - scorecardAwayAnalysis.runs)}</strong><small>{scorecardWinnerAnalysis?.name ?? "Tied result"}</small></article><article><span>WICKET EDGE</span><strong>{Math.abs(scorecardHomeAnalysis.wicketsLost - scorecardAwayAnalysis.wicketsLost)}</strong><small>difference in dismissals</small></article><article><span>EXTRAS EDGE</span><strong>{Math.abs(scorecardHomeAnalysis.extrasGiven - scorecardAwayAnalysis.extrasGiven)}</strong><small>extra deliveries conceded</small></article><article><span>SKINS EDGE</span><strong>{Math.abs(scorecardHomeAnalysis.skins - scorecardAwayAnalysis.skins)}</strong><small>skins difference</small></article></div><div className="match-analysis-grid"><article className="runs-wickets-chart"><header><div><span>RUNS VS WICKETS</span><strong>Scoring output and wicket risk</strong></div><small>Longer bar = more runs</small></header>{[scorecardHomeAnalysis, scorecardAwayAnalysis].map((side) => <div className="match-balance-row" key={side.name}><span><Initials name={side.name} src={team && side.name.toLowerCase() === team.name.toLowerCase() ? team.imageUrl : null} /><b>{side.name}</b></span><div><i><em style={{ width: `${side.runs / scorecardRunScale * 100}%` }} /></i><strong>{side.runs}<small>RUNS</small></strong></div><b>{side.wicketsLost}<small>WKTS LOST</small></b></div>)}</article><article className="extras-pressure-chart"><header><div><span>BOWLING DISCIPLINE</span><strong>Extras given away &amp; dot pressure</strong></div><small>Lower extras is better</small></header>{[scorecardHomeAnalysis, scorecardAwayAnalysis].map((side) => <div className="discipline-row" key={side.name}><span><b>{side.name}</b><small>{side.boundaries} boundaries faced · {side.wicketsTaken} wickets taken</small></span><strong>{side.extrasGiven}<small>EXTRAS</small></strong><div><i><em style={{ width: `${side.dotPressure}%` }} /></i><b>{side.dotPressure}%<small>DOT PRESSURE</small></b></div></div>)}</article><article className="run-type-card"><header><div><span>RUN TYPES</span><strong>How our runs were built</strong></div><small>{scorecardRunTypeTotal} outcomes</small></header><div><div className="team-run-donut" role="img" aria-label={`${team?.name ?? "Our team"} run type distribution this match`} style={{ background: `conic-gradient(${scorecardRunTypeGradient || "#e8dfcf 0 100%"})` }}><span><strong>{Math.round((scorecardRunTypes.ones + scorecardRunTypes.twos + scorecardRunTypes.threes) / scorecardRunTypeTotal * 100)}%</strong><small>ROTATION</small></span></div><div className="team-run-legend">{scorecardRunTypeEntries.map((item) => <span key={item.label}><i style={{ background: item.color }} /><b>{item.label}</b><strong>{item.value}</strong><small>{Math.round(item.value / scorecardRunTypeTotal * 100)}%</small></span>)}</div></div></article><article className="wicket-risk-card"><header><div><span>WICKET RISK</span><strong>How our wickets fell</strong></div><small>{scorecardDismissalTotal} dismissals</small></header><div><div className="wicket-risk-donut" role="img" aria-label={`${team?.name ?? "Our team"} dismissal pattern this match`} style={{ background: `conic-gradient(${scorecardDismissalGradient || "#eee7df 0 100%"})` }}><span><strong>{scorecardDismissalEntries.reduce((sum, [, count]) => sum + count, 0)}</strong><small>OUTS</small></span></div><div className="wicket-risk-legend">{scorecardDismissalEntries.length ? scorecardDismissalEntries.map(([label, count], index) => <span key={label}><i style={{ background: scorecardDismissalPalette[index % scorecardDismissalPalette.length] }} /><b>{label}</b><strong>{count}</strong></span>) : <small>No dismissals recorded.</small>}</div></div></article></div></section>
        ...
        {view === "players" && <>
          ...
          <div className="player-directory-grid">{playerDirectoryRows.map((item) => { const stats = directoryStatsFor(item); const fingerprint = directoryFingerprintFor(stats); const viewName = playerDirectorySeasonFilter === "all" ? "ALL TIME" : (seasonNames[Number(playerDirectorySeasonFilter)] ?? "SELECTED SEASON").toUpperCase(); const currentSeasonEntry = item.seasons.find((entry) => entry.teamSeasonId === currentSeason?.id); return <button className="player-directory-card" key={item.id} onClick={() => openPublicPlayer(item.id)}><div className="directory-card-head"><Initials name={item.displayName} large src={item.imageUrl} /><span><em>PLAYER PROFILE · {viewName}</em><strong>{item.displayName}</strong><small>{item.seasons[0]?.teamName ?? team?.name ?? "Action Cricket"} · permanent career profile</small>{currentSeasonEntry && <FinalsBadge games={currentSeasonEntry.games} totalMatches={currentSeasonMatchCount} />}</span><div className="directory-card-fingerprint" aria-hidden="true"><svg viewBox="0 0 56 56"><circle className="dcf-track" cx="28" cy="28" r="25" /><circle className="dcf-track" cx="28" cy="28" r="19" /><circle className="dcf-track" cx="28" cy="28" r="13" /><circle className="dcf-value dcf-batting" cx="28" cy="28" r="25" strokeDasharray={`${2 * Math.PI * 25 * fingerprint.battingScore / 100} ${2 * Math.PI * 25}`} /><circle className="dcf-value dcf-bowling" cx="28" cy="28" r="19" strokeDasharray={`${2 * Math.PI * 19 * fingerprint.bowlingScore / 100} ${2 * Math.PI * 19}`} /><circle className="dcf-value dcf-impact" cx="28" cy="28" r="13" strokeDasharray={`${2 * Math.PI * 13 * fingerprint.impactScore / 100} ${2 * Math.PI * 13}`} /></svg><strong>{fingerprint.overall}</strong></div></div><div className="directory-stats"><span><b>{stats.games}</b><small>GAMES</small></span><span><b>{stats.runs}</b><small>RUNS</small></span><span><b>{stats.strikeRate}</b><small>STRIKE RATE</small></span><span><b>{stats.wickets}</b><small>WKTS</small></span></div><div className="directory-card-foot"><span>{stats.scorecards ? `${viewName.toLowerCase()} match history connected` : "Waiting for first scorecard in this view"}</span><b>View full profile →</b></div></button>; })}</div>
        </>}
