...
function TeamPerformanceCharts({ teamName, matches, sortOrder }: { teamName: string; matches: MatchActivity[]; sortOrder: TeamStatsSort }) {
  ...
  return <div className="team-intelligence-suite">
    ...
    <section className="team-pattern-grid">
      <article className="consistency-card run-rate-card"><header><div><p>RUN RATE TREND</p><h2>Our run rate vs opponent</h2></div><div className="chart-legend"><i className="legend-for" />Our RR<i className="legend-against" />Opponent RR</div></header><div className="run-rate-chart"><div className="run-rate-scroll"><svg viewBox={`0 0 ${runRateChartWidth} 100`} preserveAspectRatio="none" style={{ width: `${runRateChartWidth}px` }} role="img" aria-label={`${teamName} run rate versus opponent run rate across ${runRateSeries.length} matches`}>
        <defs>
          <linearGradient id="rrGradFor" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--navy)" stopOpacity="0.28" /><stop offset="100%" stopColor="var(--navy)" stopOpacity="0" /></linearGradient>
          <linearGradient id="rrGradAgainst" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--orange)" stopOpacity="0.22" /><stop offset="100%" stopColor="var(--orange)" stopOpacity="0" /></linearGradient>
        </defs>
        ...
        {runRateSeries.map((match, index) => <g key={`${match.id}-against`} className="run-rate-point"><circle className="run-rate-dot run-rate-dot-against" r="5" cx={runRatePoint(match.theirRate, index).split(",")[0]} cy={runRatePoint(match.theirRate, index).split(",")[1]} /><text className="run-rate-tip run-rate-tip-against" x={runRatePoint(match.theirRate, index).split(",")[0]} y={Number(runRatePoint(match.theirRate, index).split(",")[1]) - 8}>{`${match.opponent} · ${match.theirRate} rpo`}</text></g>)}
        {runRateSeries.map((match, index) => <g key={`${match.id}-for`} className="run-rate-point"><circle className="run-rate-dot run-rate-dot-for" r="5" cx={runRatePoint(match.ourRate, index).split(",")[0]} cy={runRatePoint(match.ourRate, index).split(",")[1]} /><text className="run-rate-tip run-rate-tip-for" x={runRatePoint(match.ourRate, index).split(",")[0]} y={Number(runRatePoint(match.ourRate, index).split(",")[1]) - 8}>{`${teamName} · ${match.ourRate} rpo vs ${match.opponent}`}</text></g>)}
      </svg><div className="run-rate-labels" style={{ width: `${runRateChartWidth}px` }}>{runRateSeries.map((match, index) => <span key={match.id} title={match.opponent} style={{ left: `${runRateCoord(0, index)[0]}px` }}>{index + 1}</span>)}</div></div></div></article>
