...
function TeamPerformanceCharts({ teamName, matches, sortOrder }: { teamName: string; matches: MatchActivity[]; sortOrder: TeamStatsSort }) {
  ...
  const runRatePoint = (rate: number, index: number) => `${runRateSeries.length > 1 ? index / (runRateSeries.length - 1) * 100 : 50},${100 - Math.min(96, rate / maxRunRate * 90)}`;
  ...
  return <div className="team-intelligence-suite">
    ...
    <section className="team-pattern-grid">
      <article className="consistency-card run-rate-card"><header><div><p>RUN RATE TREND</p><h2>Our run rate vs opponent</h2></div><div className="chart-legend"><i className="legend-for" />Our RR<i className="legend-against" />Opponent RR</div></header><div className="run-rate-chart"><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label={`${teamName} run rate versus opponent run rate across ${runRateSeries.length} matches`}>
        ...
        <polyline className="run-rate-line run-rate-line-for" points={runRateSeries.map((match, index) => runRatePoint(match.ourRate, index)).join(" ")} />
        <polyline className="run-rate-line run-rate-line-against" points={runRateSeries.map((match, index) => runRatePoint(match.theirRate, index)).join(" ")} />
        {runRateSeries.map((match, index) => <circle key={`${match.id}-for`} className="run-rate-dot run-rate-dot-for" r="1.6" cx={runRatePoint(match.ourRate, index).split(",")[0]} cy={runRatePoint(match.ourRate, index).split(",")[1]}><title>{`${teamName} ${match.ourRate} runs/over vs ${match.opponent}`}</title></circle>)}
        {runRateSeries.map((match, index) => <circle key={`${match.id}-against`} className="run-rate-dot run-rate-dot-against" r="1.6" cx={runRatePoint(match.theirRate, index).split(",")[0]} cy={runRatePoint(match.theirRate, index).split(",")[1]}><title>{`${match.opponent} ${match.theirRate} runs/over`}</title></circle>)}
