...
function TeamPerformanceCharts({ teamName, matches, sortOrder }: { teamName: string; matches: MatchActivity[]; sortOrder: TeamStatsSort }) {
  ...
  const runRateCoord = (rate: number, index: number) => [runRateSeries.length > 1 ? index / (runRateSeries.length - 1) * 100 : 50, 100 - Math.min(96, rate / maxRunRate * 90)];
  ...
  const runRateAreaFor = runRateSeries.length ? `${runRatePathFor} L${runRateCoord(0, runRateSeries.length - 1)[0]},100 L0,100 Z` : "";
  const runRateAreaAgainst = runRateSeries.length ? `${runRatePathAgainst} L${runRateCoord(0, runRateSeries.length - 1)[0]},100 L0,100 Z` : "";
  ...
  return <div className="team-intelligence-suite">
    ...
    <section className="team-pattern-grid">
      <article className="consistency-card run-rate-card"><header><div><p>RUN RATE TREND</p><h2>Our run rate vs opponent</h2></div><div className="chart-legend"><i className="legend-for" />Our RR<i className="legend-against" />Opponent RR</div></header><div className="run-rate-chart"><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label={`${teamName} run rate versus opponent run rate across ${runRateSeries.length} matches`}>
        ...
        {[0, 25, 50, 75, 100].map((y) => <line key={y} x1="0" x2="100" y1={y} y2={y} className="run-rate-grid" />)}
        ...
        {runRateSeries.map((match, index) => <g key={`${match.id}-against`} className="run-rate-point"><circle className="run-rate-dot run-rate-dot-against" r="1.8" cx={runRatePoint(match.theirRate, index).split(",")[0]} cy={runRatePoint(match.theirRate, index).split(",")[1]} /><text className="run-rate-tip run-rate-tip-against" x={runRatePoint(match.theirRate, index).split(",")[0]} y={Number(runRatePoint(match.theirRate, index).split(",")[1]) - 4}>{`${match.opponent} · ${match.theirRate} rpo`}</text></g>)}
        {runRateSeries.map((match, index) => <g key={`${match.id}-for`} className="run-rate-point"><circle className="run-rate-dot run-rate-dot-for" r="1.8" cx={runRatePoint(match.ourRate, index).split(",")[0]} cy={runRatePoint(match.ourRate, index).split(",")[1]} /><text className="run-rate-tip run-rate-tip-for" x={runRatePoint(match.ourRate, index).split(",")[0]} y={Number(runRatePoint(match.ourRate, index).split(",")[1]) - 4}>{`${teamName} · ${match.ourRate} rpo vs ${match.opponent}`}</text></g>)}
      </svg></div><div className="run-rate-labels">{runRateSeries.map((match, index) => <span key={match.id} title={match.opponent}>{index + 1}</span>)}</div></article>
