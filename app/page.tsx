...
function TeamPerformanceCharts({ teamName, matches, sortOrder }: { teamName: string; matches: MatchActivity[]; sortOrder: TeamStatsSort }) {
  ...
  const teamRunTypes = { dots: 0, ones: 0, twos: 0, threes: 0, boundaries: 0, dismissals: 0 };
      ...
      for (const pair of innings.pairs ?? []) {
        for (const over of pair.overs ?? []) {
          ...
          for (const delivery of over.deliveries ?? []) {
            ...
            else if ((runs ?? 0) >= 4) teamRunTypes.boundaries += 1;
  ...
  const runTypeEntries = [
    { label: "Dots", value: teamRunTypes.dots, color: "#a8b3c4" }, { label: "1s", value: teamRunTypes.ones, color: "#63748a" }, { label: "2s", value: teamRunTypes.twos, color: "#347ff0" }, { label: "3s", value: teamRunTypes.threes, color: "#5f5bea" }, { label: "4+", value: teamRunTypes.boundaries, color: "#a248ed" }, { label: "Outs", value: teamRunTypes.dismissals, color: "#ff4d0a" },
  ];
...
function PlayerPerformanceCharts({ player, activities, seasonFilter, onSeasonFilterChange, seasonNames, nameHeading = "h1" }: { player: PlayerProfile; activities: MatchActivity[]; seasonFilter: string; onSeasonFilterChange: (seasonId: string) => void; seasonNames: Record<number, string>; nameHeading?: "h1" | "h2" }) {
  ...
  return <div className="smart-player-suite">
    <section className="smart-athlete-hero">
      <div className="smart-athlete-id"><small>{player.role || "All-rounder"}</small><PlayerNameHeading><b>{jersey}</b>{displayName}</PlayerNameHeading><em>{teamName}</em><p>Every season and uploaded appearance builds one permanent performance story.</p></div>
...
export default function Home() {
  ...
  const [scorecardImportMethod, setScorecardImportMethod] = useState<"html" | "url">("html");
  ...
  }, [players, rankingSearch, rankingParticipationFilter, rankingTableSort, rankingSortDirection]);
  ...
  async function uploadMatches(event: FormEvent) {
    ...
    if (scorecardImportMethod === "html" && !scorecardFiles.length) return fail("Choose one or more completed HTML scorecards first.");
    if (scorecardImportMethod === "url" && !scorecardUrl.trim()) return fail("Paste an Action Sport scorecard URL first.");
    ...
    if (scorecardImportMethod === "html") scorecardFiles.forEach((file) => form.append("scorecards", file));
    else form.append("scoresheetUrl", scorecardUrl.trim());
    try {
      ...
      await refreshCalculatedStats();
      setScorecardFiles([]); setScorecardUrl(""); setUploadInputKey((key) => key + 1);
  ...
  return <main className="app-root">
    ...
    <div className="app-layout">
      ...
      <section className="main-column">
          ...
          {team ? <>
            ...
            <div className="home-dashboard-grid">
              <section className="home-latest-card">
                ...
                {homeLatestMatch ? <><div className="home-result-score"><span><Initials name={team.name} /><b>{team.name}</b><strong>{homeLatestTeamScore}</strong></span><i><b className={homeLatestOutcome.toLowerCase()}>{homeLatestOutcome}</b><small>{homeLatestMatch.playedAt}</small></i><span><strong>{homeLatestOpponentScore}</strong><b>{homeLatestOpponent}</b><Initials name={homeLatestOpponent} /></span></div><div className="home-match-readout"><span><small>MATCH TYPE</small><b>{matchTypeLabel(homeLatestMatch.matchType)}</b></span><span><small>RUN MARGIN</small><b>{Math.abs(homeLatestTeamScore - homeLatestOpponentScore)}</b></span><span><small>TOP IMPACT</small><b>{homeLatestTopPerformance?.playerName ?? "—"}</b></span><span><small>CONTRIBUTION</small><b>{homeLatestTopPerformance?.contribution ?? "—"}</b></span></div></> : <div className="home-panel-empty"><strong>No completed result yet.</strong><span>Upload the first scorecard and the season story will start here.</span><button onClick={() => openTeamSection("scorecard-imports")}>Update scores</button></div>}
              </section>
        ...
        {view === "team" && <>
          ...
          {team && scorecardImportSeason && <form id="scorecard-imports" className="quick-sync-card team-admin-import team-admin-import-top file-upload-card" onSubmit={uploadMatches}>
            ...
            <div className="scorecard-import-control">
              ...
              <div className="import-method-switch"><button type="button" className={scorecardImportMethod === "html" ? "active" : ""} onClick={() => setScorecardImportMethod("html")}>HTML files</button><button type="button" className={scorecardImportMethod === "url" ? "active" : ""} onClick={() => setScorecardImportMethod("url")}>Scorecard URL</button></div>
              ...
              {scorecardImportMethod === "html" ? <label className="scorecard-file-picker"><input key={uploadInputKey} aria-label="Completed HTML scorecards" type="file" accept=".html,.htm,text/html" multiple required onChange={(event) => setScorecardFiles(Array.from(event.target.files ?? []))} /><span>{scorecardFiles.length ? `${scorecardFiles.length} scorecard${scorecardFiles.length === 1 ? "" : "s"} selected` : "Choose HTML files"}</span></label> : <label className="scorecard-url-field"><span>Action Sport URL</span><input required type="url" value={scorecardUrl} onChange={(event) => setScorecardUrl(event.target.value)} placeholder="https://actionsport.spawtz.com/...FixtureId=2336793" /></label>}
            </div>
        ...
        {view === "players" && <>
          <div className="page-intro"><div><p className="overline">PLAYER DIRECTORY</p><h1>Every {team?.name ?? "team"} player has a page.</h1><p>Every roster player uses one permanent profile. Season and all-time statistics come only from imported scorecards.</p></div><span className="captain-badge">{teamDirectory.length} PLAYER PAGES</span></div>
        ...
        {view === "player" && viewedProfile && <>
          ...
          <section className="history-panel"><div className="section-title"><div><p className="overline">CLUB HISTORY</p><h2>Seasons and teams</h2></div></div>{viewedProfile.seasons.length ? viewedProfile.seasons.map((item) => <div className="history-row" key={item.id}><Initials name={item.teamName} /><span><b>{item.teamName}</b><small>Season {item.seasonId} · {item.active ? "Active squad" : "Former player"}</small></span><strong>{item.games}<small>GAMES</small></strong><strong>{item.runs}<small>RUNS</small></strong><strong>{item.wickets}<small>WKTS</small></strong></div>) : <div className="panel-empty">No connected team seasons yet.</div>}</section>
          <section className="history-panel"><div className="section-title"><div><p className="overline">MATCH HISTORY</p><h2>{playerSeasonFilter === "all" ? "Every linked appearance" : "Selected season appearances"}</h2></div><span>{viewedProfileMatches.length} scorecards</span></div>{viewedProfileMatches.length ? viewedProfileMatches.map((item) => <button type="button" className="history-row" key={item.claimId} onClick={() => { const match = activities.find((activity) => activity.fixtureId === item.fixtureId); if (match) openScorecard(match.id); }}><Initials name={item.teamName} /><span><b>{item.homeTeam} {item.homeScore}–{item.awayScore} {item.awayTeam}</b><small>{item.playedAt} · Fixture {item.fixtureId}</small></span><strong>{item.runs}<small>RUNS</small></strong><strong>{item.wickets}<small>WKTS</small></strong><strong>{item.contribution}<small>IMPACT</small></strong></button>) : <div className="panel-empty">No linked scorecards in this season yet.</div>}</section>
        </>}
        ...
        {view === "leaderboards" && <>
          <section className="rankings-hero">
            ...
            <header><div className="rankings-team-id"><Initials name={team?.name ?? "ActionHQ"} /><span><small>TEAM &amp; PLAYER INTELLIGENCE</small><strong>{team?.name ?? "Your team"}</strong></span></div><label className="rankings-season-select"><span>VIEWING SEASON</span>{team?.seasons?.length ? <select aria-label="Choose ranking season" value={season?.id ?? ""} onChange={(event) => setSelectedSeasonId(Number(event.target.value))}>{team.seasons.map((item) => <option key={item.id} value={item.id}>{item.name || `Season ${item.externalSeasonId}`}</option>)}</select> : <b>Current season</b>}</label></header>
            ...
            <div className="rankings-season-kpis"><article><span>PLAYERS</span><strong>{players.length}</strong><small>on this season roster</small></article><article><span>SCORECARDS</span><strong>{seasonActivities.length}</strong><small>verified results</small></article><article className="positive"><span>RECORD</span><strong>{seasonTeamSummary.wins}–{seasonTeamSummary.losses}</strong><small>{seasonTeamSummary.draws} draw{seasonTeamSummary.draws === 1 ? "" : "s"}</small></article><article><span>AVG SCORE</span><strong>{seasonTeamSummary.averageScored}</strong><small>{seasonTeamSummary.averageConceded} conceded</small></article></div>
          </section>

          {players.length ? <>
            <section className="ranking-leader-strip" aria-label="Season category leaders">
              {[
                { label: "Impact leader", player: players[0], value: rankingMetricDisplay(players[0], "impact"), unit: "CONTRIBUTION", tone: "impact" },
                { label: "Top scorer", player: topRuns[0], value: String(topRuns[0]?.runs ?? 0), unit: "RUNS", tone: "runs" },
                { label: "Wicket leader", player: topWickets[0], value: String(topWickets[0]?.wickets ?? 0), unit: "WICKETS", tone: "wickets" },
            ...
            <section className="ranking-board-card">
              <header className="ranking-board-heading"><div><p className="overline">LIVE LEADERBOARD</p><h2>Choose what matters</h2><span>The podium and order update instantly. Every number comes from this season’s scorecards.</span></div><button className="outline-button" onClick={() => navigateTo("players")}>View player profiles</button></header>
              <div className="ranking-metric-switch" aria-label="Choose leaderboard metric">{rankingMetricOptions.map((metric) => <button key={metric.id} aria-pressed={rankingMetric === metric.id} onClick={() => setRankingMetric(metric.id)}><b>{metric.short}</b><span>{metric.label}</span></button>)}</div>
            ...
            <section className="ranking-table-card">
              ...
              <div className="ranking-table-status"><span><b>{rankingTablePlayers.length}</b> of {players.length} players shown</span>{rankingFiltersActive && <button onClick={resetRankingTable}>Reset board</button>}</div>
