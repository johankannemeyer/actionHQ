  ...
  return <PlayerImageContext.Provider value={playerImages}><main className="app-root">
    ...
    <div className="app-layout">
      ...
      <section className="main-column">
        ...
        {view === "team" && <>
          <div className="page-intro"><div><p className="overline">TEAM ADMINISTRATION</p><div className="admin-team-heading"><Initials name={team?.name ?? "Die Bron"} large src={team?.imageUrl ?? null} /><h1>{team?.name ?? "Die Bron"}</h1></div><p>Import completed games first, then manage the season, official roster and stored results for this team.</p>{team && <div className="admin-logo-actions"><label>{team.imageUrl ? "Change logo" : "Upload logo"}<input type="file" accept="image/*" onChange={async (event) => { const file = event.target.files?.[0]; event.target.value = ""; if (!file) return; try { await saveTeamImage(await readImageThumbnail(file)); } catch { setNotice("Could not read that image."); } }} /></label>{team.imageUrl && <button type="button" disabled={working === "account"} onClick={() => saveTeamImage(null)}>Remove logo</button>}</div>}</div></div>
